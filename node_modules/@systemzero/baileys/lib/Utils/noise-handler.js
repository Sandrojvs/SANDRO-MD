...
export const makeNoiseHandler = ({ keyPair: { private: privateKey, public: publicKey }, NOISE_HEADER, logger, routingInfo }) => {
    ...
    // decodeFrame mutates shared state (inBytes, read/write counters) and awaits
    // async decompression/decoding internally. Incoming WebSocket "message" events
    // are not serialized by the caller, so overlapping calls (e.g. during a burst of
    // messages) can interleave and corrupt inBytes/counters, leading to misparsed
    // frames. Chain calls through this mutex so each decodeFrame fully completes
    ...
    return {
        ...
        decodeFrame: (newData, onFrame) => {
            ...
            // keep the chain alive even if this job fails, so later frames still get processed;
            // the failure is still surfaced to the caller via the returned (unswallowed) job promise
            mutex = job.catch(() => undefined);
            return job;
        }
    };
};
